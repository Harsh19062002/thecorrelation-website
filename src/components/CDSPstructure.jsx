...
import { BarChart3, Brain, Zap, Code2 } from 'lucide-react';
  ...
  const roadmap = [
    ...
    },
    {
      ...
      points: [
        ...
        'Sklearn, model validation',
        'ML project using real data',
      ],
    },
    {
    ...
    <section className="py-20 bg-white" ref={containerRef}>
      <div className="container mx-auto px-6">
        <h2 className="text-4xl md:text-5xl font-bold text-center mb-16 text-gray-800">
          📚 Curriculum Roadmap – 9 Months to Success
        </h2>
        ...
        <div className="relative max-w-6xl mx-auto timeline-line">
          ...
          <div className="space-y-16">
            {roadmap.map((step, index) => {
              ...
              return (
                <div key={index} className="timeline-item flex flex-col md:flex-row items-center gap-8 opacity-30 blur-sm translate-y-8 transition-all duration-700">
                  <div className={`md:w-1/2 ${isLeft ? 'md:text-right' : ''}`}>
                    {isLeft && (
                      <div className="bg-gray-50 rounded-lg shadow-md p-6">
                        ...
                        <div className={`mt-3 text-xs font-semibold ${step.color.split(' ')[1]}`}>{step.weeks}</div>
                      </div>
                    )}
                  </div>

                  <div className={`timeline-icon relative z-10 ${step.color.split(' ')[0]} w-16 h-16 rounded-full flex items-center justify-center shadow-lg`}>
                    {step.icon}
                  </div>
